   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      Begin VB.Frame Frame13 
         ...
         Begin VB.Label Label1 
            ...
            AutoSize        =   -1  'True
            BackStyle       =   0  '透明
            Caption         =   "快速確認僅限麒麟、雀巢、維他露、亞培...等貨主！"
            BeginProperty Font 
               Name            =   "新細明體"
               Size            =   11.25
               Charset         =   136
...
str_SQL = "Update SDN02T Set CustSignDate = '" & dtp_OneOrder_SignDate.Value & "', invback = '" & cboInvBack.Text & "'," & _
          " Confirm_UserID = '" & User_id & "',Confirm_Date = '" & Format(Now, "yyyy/mm/dd hh:nn:ss") & "',Confirm_Notes = '正常訂單' , CustomerOrderkey1 ='" & txt_OneOrder_CustomerOrderkey1.Text & "', Scan = '" & cmbScan.Text & "' " & _
          ", sdnback = 1 " & _
...
str_SQL = "Update SDN02T " & _
            "Set CustSignDate = '" & dtp_OneOrder_SignDate.Value & "' " & _
              ...
              ",Confirm_Date = '" & Format(Now, "yyyy/mm/dd hh:nn:ss") & "',sdnsenddate = '" & Format(dtpSDNSendDate.Value, "YYYY/MM/DD") & "',Confirm_Notes = '異常訂單' , CustomerOrderkey1 ='" & txt_OneOrder_CustomerOrderkey1.Text & "', Scan = '" & cmbScan.Text & "' " & _
...
str_SQL = "Update SDN02T " & _
              ...
              ", Total_Cost = '" & txt_TotalCost.Text & "' " & _
              ", SDN_NOTE = '" & txt_SDNNote.Text & "' ,invback = '" & cboInvBack.Text & "' " & _
              ",Confirm_Date = '" & Format(Now, "yyyy/mm/dd hh:nn:ss") & "',sdnsenddate = '" & Format(dtpSDNSendDate.Value, "YYYY/MM/DD") & "',Confirm_Notes = '未出訂單' , CustomerOrderkey1 ='" & txt_OneOrder_CustomerOrderkey1.Text & "', Scan = '" & cmbScan.Text & "' " & _
              "Where Receipt_No = '" & txt_OneOrder_OrderKey.Text & "' "
...
If UCase(strOtherOrder2WMS) = "YES" And txt_OneOrder_StorerKey <> "LABT01" Then 'WMS是否新增採購單 1s

    '寫入WMS採購單
    If RTrim(txt_Priority.Text) <> "R" And RTrim(txt_Priority.Text) <> "RC" And RTrim(txt_Priority.Text) <> "A2B" Then '退貨單、提貨配送與移倉入庫不產生WMS採購單2s
        '毛寶和菲仕蘭
...
If cmbOrderkey = "" Then 'Terry 20180907 改用like查詢
    Call ClearForm
...
'
''補資料
'str_SQL = "select 分公司代碼=isnull(co.BranchId,' '),客戶訂單類別=isnull(o.externordertype,' '), " & _
'"應收=o.cash,實收=case when isnull(rtrim(cast(o.receiveCash as char)),'') = '0' then ' ' else  isnull(rtrim(cast(o.receiveCash as char)),'') end ," & _
'"組單類型 = rtrim(isnull(o.B_city,''))" & _
...
'    txt_ReceiveCash = tmp_Rs.Fields("實收")
'    txt_Cash = Val(tmp_Rs.Fields("應收"))
'    cbx_B_city = tmp_Rs.Fields("組單類型")
'Else
'    tmp_Rs.Close
'End If
...
intSDNCarChange = 0 '由簽單維護進入
frm_SDNCarNOFix.Show vbModal
End Sub

Private Sub cmdDelete_Click()
'Terry 20190328 計費刪除功能
...
MsgBox "路編內計費資料皆已刪除!", vbOKOnly, "刪除計費"
...
Do While Not rsOrderT0.EOF
    If RTrim(rsOrderT0("實收代收貨款")) <> "" And Val(rsOrderT0("實收代收貨款")) <> Val(rsOrderT0("應收代收貨款")) Then
...
dgOrderT0.Col = 0
'cn.CommandTimeout = 0
cn.Execute "update SDN01T set sdn_Date = getdate() where c_route_no = '" & rsRouteT0("二次路編") & "'", RowsAffect, adExecuteNoRecords
Screen.MousePointer = 11

Do While Not rsOrderT0.EOF

'mark by Gemini @20140312 下列狀況已無法選取，故無需再檢查
'    '簽單是否已維護
'    Call Confirm_Recordset_Closed(tmp_Rs)
'    str_SQL = "select receipt_no from sdn02t (nolock) where confirm_notes <> '' and receipt_no = '" & rsOrderT0("TMS單號") & "' "
'    tmp_Rs.Open str_SQL, cn, adOpenForwardOnly, adLockReadOnly
'
...
'    '訂單量與出貨量不符
'    Call Confirm_Recordset_Closed(tmp_Rs)
'    str_SQL = "select receipt_no from sdn03t (nolock) where order_qty <> ship_qty and receipt_no = '" & rsOrderT0("TMS單號") & "' "
'    tmp_Rs.Open str_SQL, cn, adOpenForwardOnly, adLockReadOnly
'
...
'    tmp_Rs.Close
    ...
    '更新回Orders的實收,實收空白則系統代預收，實收有值則用實收
    If RTrim(rsOrderT0("實收代收貨款")) = "" Then
        '更新預收=實收
        If Val(rsOrderT0("應收代收貨款")) > 0 Then
            cn.Execute "update o set o.receiveCash = '" & Val(rsOrderT0("應收代收貨款")) & "' from orders o join sdn02t s2 on o.orderkey = s2.c_receipt_no where s2.receipt_no = '" & rsOrderT0("TMS單號") & "' and o.type <> '刪單'", RowsAffect, adExecuteNoRecords
        End If
    Else
        '更新預收=實收
        If Val(rsOrderT0("實收代收貨款")) > 0 Then
            cn.Execute "update o set o.receiveCash = '" & Val(rsOrderT0("實收代收貨款")) & "' from orders o join sdn02t s2 on o.orderkey = s2.c_receipt_no where s2.receipt_no = '" & rsOrderT0("TMS單號") & "' and o.type <> '刪單'", RowsAffect, adExecuteNoRecords
        End If
    End If
    
    '運費計算
    str_SQL = "exec gs_Cost '" & rsOrderT0("TMS單號") & "' "
    cn.Execute str_SQL, RowsAffect, adExecuteNoRecords
    
    '應收分攤計算 Mark by Gemini @20200225 4 改到gs_coat呼叫
'    str_SQL = "exec Es_ARnoDistribution '" & rsOrderT0("TMS單號") & "' "
'    cn.Execute str_SQL, RowsAffect, adExecuteNoRecords
...
Exit Sub
err_Handle:
Call ErrorMsgbox(Me.Caption, err.Number, err.Description, "cmdDeliveryokT0_Click;TMS:" & rsOrderT0("TMS單號") & ";Tran_Level:" & Tran_Level)
End Sub
...
If rsRouteT0("選取") = "V" Then
    ...
    If rsRouteT0("車牌號碼") = "002-34" Or rsRouteT0("車牌號碼") = "002-29" Or rsRouteT0("車牌號碼") = "001-97" Or rsRouteT0("車牌號碼") = "000-31" Or rsRouteT0("車牌號碼") = "001-36" Or rsRouteT0("車牌號碼") = "000-70" Or rsRouteT0("車牌號碼") = "000-67" Or rsRouteT0("車牌號碼") = "001-23" Then MsgBox "議價應付計算，無法選取車牌號碼(" & rsRouteT0("車牌號碼") & ")!", 16, "注意": GoTo EndProc
    If Len(Trim(strCarno)) > 0 And UCase(strCarno) <> UCase(rsRouteT0("車牌號碼")) Then MsgBox "車牌號碼不同!", 16, "注意": GoTo EndProc
...
'取總應付金額 edit by Eric 20140407 改抓trp17m判斷
rsTmp.MoveFirst
Do While Not rsTmp.EOF

If UCase(rsTmp("應付不分攤")) <> "1" And rsTmp("請款類別") <> "轉運" And Left(rsTmp("備註"), 3) <> "不分攤" Then
        dbAP = dbAP + rsTmp("應付總價")
End If
...
dbPremiam = InputBox("不列入A段分攤條件如下:" & vbCr & vbLf & "1.計費代碼主擋中的應付不分攤=1" & vbCr & vbLf & "2.計費類別:轉運" & vbCr & vbLf & "3.備註開頭:不分攤", "請輸入議價金額(金額空白或按取消可中止分攤)", "")

If dbPremiam = "" Then 'Edit by Gemini @ 20201208 空白則不議價
        Recordset2Excel "議價應付分攤", rsTmp
        MyXlsApp.Visible = True
        Set MyXlsApp = Nothing
        GoTo EndProc
...
dbTRouteNO = InputBox("1.請輸入共配路線編號，並按確認" & vbCr & vbLf & "2.如要清空共配路編，請輸入空白並按確認" & vbCr & vbLf & "3.如不紀錄共配路編，請按取消", "共配路線編號", "")
...
'計算議價 edit by Eric 20140407 改抓trp17m判斷
str_SQL = "Update sdn05t " & _
          "Set sdn05t.Premiam = sdn05t.sumpayable / " & dbAP & " * " & dbPremiam & _
          ",sdn05t.note = sdn05t.note + '_" & strCarno & " 專車價(" & dbPremiam & ")' " & _
          ...
          "and sdn05t.costkind <> ('轉運') " & _
          "and left(isnull(sdn05t.note,''),3) <> '不分攤'" & _
...
Dim strTmp As String
    ...
    For i = 0 To Car_Num.ListCount - 1
        ...
        End If
    Next
    If Len(strTmp) > 0 Then str_Where = str_Where & " and 車號 in (" & Left(strTmp, Len(strTmp) - 1) & ") "
...
'LABT01則更新組單類型
If RTrim(txt_OneOrder_StorerKey) = "LABT01" Then cn.Execute "update o set o.b_city = '" & RTrim(cbx_B_city.Text) & "' from orders o join sdn02t s2 on o.orderkey = s2.c_receipt_no where s2.receipt_no = '" & RTrim(frm_OP_SDNConfirm.txt_OneOrder_OrderKey.Text) & "' and o.type <> '刪單' and o.storerkey = 'LABT01'", RowsAffect, adExecuteNoRecords
...
str_SQL = "select distinct 選取 = ' ' " & _
        ...
        ",出貨板數 = round( sum(case when isnull(sp.pallet,0) = 0 then 0 else t3.ship_qty /sp.pallet end) ,3) " & _
        ",出貨箱數 = sum(case when isnull(sp.casecnt,0) = 0 then 0 else (t3.ship_qty /sp.casecnt) end) " & _
        ",出貨材積 = round( sum(t3.ship_qty * sp.stdcube),3) " & _
        ",出貨重量 = round( sum(t3.ship_qty * sp.stdgrosswgt),3) " & _
...
str_SQL = str_SQL & str_Where & "group by convert(Char(8), t1.delivery_date, 112),t1.c_route_no,rtrim(t1.c_vehicle_id_no),rtrim(t1.driver),rtrim(isnull(t1.receiver,'')),rtrim(t1.adduser),t1.adddate order by 出車日期,二次路編"
...
'匯出Excel
str_SQL = "select 計費類別 = case when s5.Costcode is null then '未計費' else '' end ,貨主 = rtrim(s2.storerkey) " & _
",簽單日 = isnull(convert(varchar(8),s1.SDN_DATE,112),convert(varchar(8),s2.CONFIRM_DATE,112)) " & _
",出車日 = convert(varchar(8),s1.DELIVERY_DATE,112),訂單日 = s2.receipt_DATE " & _
",到貨日 = s2.arrive_DATE,二次路編 = s1.C_Route_No,車號 = rtrim(s1.C_VEHICLE_ID_NO) " & _
",駕駛 = rtrim(s1.driver),請款人 = rtrim(isnull(s1.receiver,s1.driver)),路線編號 = s2.Route_No " & _
",一次車號 = rtrim(s2.VEHICLE_ID_NO),貨主單號 = rtrim(isnull(s2.extern,'')) ,TMS單號 = rtrim(isnull(s2.receipt_no,'')) " & _
",客戶編號 = rtrim(s2.consigneekey) ,客戶名稱 = rtrim(isnull(s2.cust_name,'')),單位 = rtrim(isnull(s5.Uom,'')) " & _
",數量 = isnull(s5.ChargeQty,'0'),應收單價 = isnull(s5.Receivable,'0'),應付單價 = isnull(s5.Payable,'0') " & _
",項次 = rtrim(isnull(s5.sdn_name,'')),原因 = rtrim(isnull(s5.Reason,'')),應收總價 = isnull(s5.SumReceivable,0) " & _
",應付總價 = isnull(s5.SumPayable,0) " & _
",實付總價 = case when isnull(s5.Premiam,'0') > 0 then isnull(s5.Premiam,'0') else isnull(s5.SumPayable,0) end " & _
",起點 = rtrim(isnull(s5.AreaStart,'')),迄點 = rtrim(isnull(s5.AreaEnd,'')),備註 = rtrim(isnull(s5.Note,'')) " & _
",請款類別 = rtrim(isnull(s5.CostKind,'')),請款代碼 = rtrim(isnull(s5.Costcode,'')) ,簽單回傳日期 = isnull(s2.sdnsenddate,'') " & _
",最終確認 = rtrim(isnull(s2.confirm_userid,'')) ,確認時間 = isnull(s2.confirm_date,''),簽單狀態 = rtrim(s2.confirm_notes) " & _
",簽單備註 = isnull(s2.sdn_note,''),計費時間 = isnull(s2.confirm_date,''),應付不分攤 = isnull(t17.apnodistribution,'') " & _
",客戶群組 = rtrim(isnull(t1m.CustGroup,'')),通路型態 = rtrim(isnull(t1m.CHANNEL_TYPE,'')) " & _
...
'此功能尚未確認上線
''簽單已回除Admin外不可執行
'If cmdSDNBack.Caption = "簽單已回" Then
'    If blAdmin = True Then
'        If MsgBox("是否再次確認簽單已回?", vbOKCancel, "系統管理員權限") <> vbOK Then Exit Sub
'    End If
'End If
...
ElseIf txt_OneOrder_StorerKey = "LYFY09" And Left(txt_C_ROUTE_NO, 1) <> "R" Then
    ...
    'YFY補印出貨單
    str_SQL = "select * from ev_ReportShipNotesYFY Where TMS單號 = '" & txt_OneOrder_OrderKey & "' "
    tmp_Rs.Open str_SQL, cn
    ...
    cnAccess.CommitTrans: Tran_Level = 0
    Call DB_Disconnect(cnAccess)
    MSAccessAP.DoCmd.OpenReport "YFY出貨單", acViewPreview
    MSAccessAP.DoCmd.Maximize
    MSAccessAP.Visible = True
...
'    str_SQL = "Select * From gv_ReportShipNotes where 佰事達單號 = '" & txt_OneOrder_OrderKey & "' "
    ...
    str_SQL = "select 貨主 = RTrim(t1m.storerkey) " & _
    ",貨主名稱 = (select rtrim(t16.c_name) from trp16m t16 where t16.storerkey = t2.storerkey ) " & _
    ",路線編號  = rtrim(t2.route_no),二次排車路編 = (select isnull(trp01t.c_route_no,'') from trp01t trp01t where t2.route_no = trp01t.route_no) " & _
    ",出車日期 = (select convert(varchar(8),trp01t.delivery_date,112) from trp01t trp01t where t2.route_no = trp01t.route_no) " & _
    ",需求日期 = convert(varchar(8),t2.arrive_date,112),貨主單號 = rtrim(t2.extern),佰事達單號 = t2.receipt_no " & _
    ",採購編號 = isnull(o.customerorderkey,''),客戶名稱 = rtrim(t1m.short_name),客戶地址 = rtrim(t1m.address),電話 = rtrim(t1m.phone) " & _
    ",客戶需求 = cast(isnull(t1m.notes,'') as varchar(300)),備註 = cast(o.notes as varchar(300)) " & _
    ",駕駛 = (select rtrim(isnull(trp05t.driver,'')) from trp05t trp05t where t2.route_no = trp05t.route_no) " & _
    ",車號 = rtrim(t2.vehicle_id_no) +  '(補印)',項次 = rtrim(isnull(t3.seq_no,'')),貨號 = Rtrim(t3.product_no) " & _
    ",品名 = Isnull(Rtrim(sp.Descr),''),倉別 = Isnull(Rtrim(od.lottable06),'') " & _
    ",出貨箱數 = case when sp.casecnt = 0 then 0 else floor(t3.ship_qty/sp.Casecnt) end,大包裝 = isnull(rtrim(sp.busr3),'箱') " & _
    ",出貨個數 = case when sp.casecnt = 0 then t3.ship_qty else cast(t3.ship_qty as int)%cast(sp.Casecnt as int) end , 小包裝 = isnull(rtrim(sp.busr1),'個') " & _
    ",總個數 = t3.ship_qty,件數 = case when isnull(t2.otqty,0) = 0 then '' else '共 '+rtrim(t2.otqty)+'件' end " & _
...
If rsRouteT0("選取") = "V" Then
    ...
    If rsRouteT0("車牌號碼") = "001-97" Or rsRouteT0("車牌號碼") = "000-31" Or rsRouteT0("車牌號碼") = "001-36" Or rsRouteT0("車牌號碼") = "000-70" Or rsRouteT0("車牌號碼") = "000-67" Or rsRouteT0("車牌號碼") = "001-23" Then MsgBox "應付計算，無法選取車牌號碼(" & rsRouteT0("車牌號碼") & ")!", 16, "注意": GoTo EndProc
    If Len(Trim(strCarno)) > 0 And UCase(strCarno) <> UCase(rsRouteT0("車牌號碼")) Then MsgBox "車牌號碼不同!", 16, "注意": GoTo EndProc
    strCarno = rsRouteT0("車牌號碼")
    strRoute = strRoute & rsRouteT0("二次路編") & "','"
...
str_SQL = "select 二次路編 = s2.c_route_no, 貨主 = rtrim(s2.storerkey),到貨日期 = s2.arrive_date,二次車號 = rtrim(s1.c_vehicle_id_no),一次車號 = rtrim(s2.vehicle_id_no),訂單號碼 = rtrim(s2.extern) " & _
            ",TMS單號 = rtrim(s2.receipt_no),客戶名稱 = rtrim(s2.cust_name),區域 = rtrim(t2m.city),地址 = rtrim(t1m.address),產品別 = rtrim(isnull(sp.susr1,'')),項次=rtrim(s3.seq_no),品號 = rtrim(s3.product_no) " & _
            ",件數確認 = isnull(t2.otqty,0) + isnull(o2.otqty,0),出貨箱數 = case when sp.casecnt = 0 then 1 else ceiling(s3.ship_qty/sp.casecnt) end " & _
            ",出貨總個數 = s3.ship_qty,箱入數 = sp.casecnt,單位重 = sp.stdgrosswgt,單位材 = sp.stdcube,出貨重 = sp.stdgrosswgt*ship_qty,出貨材 = sp.stdcube*ship_qty,換算材 = round( " & _
            "case when s3.storerkey = 'LTKK01' and sp.susr1 not in ('廣告品') then s3.ship_qty * sp.stdgrosswgt /12.0 " & _
            "when s3.storerkey = 'LVTL01' and sp.susr1 in ('金酒','紅酒') then s3.ship_qty * sp.stdgrosswgt /12.0 " & _
            ...
            "case when rtrim(t2m.city) in ('基隆','苗栗') then 1.1 " & _
            "when rtrim(t2m.city) in ('台北','新竹') then 1 " & _
            "when rtrim(t2m.city) = '桃園' then 0.8 else 1 end,3) " & _
...
str_SQL = str_SQL & "union all select 二次路編 = s2.c_route_no,貨主 = rtrim(s2.storerkey),到貨日期 = s2.arrive_date,二次車號 = rtrim(s1.c_vehicle_id_no),一次車號 = rtrim(s2.vehicle_id_no) " & _
                    ",訂單號碼 = rtrim(s2.extern),TMS單號 = rtrim(s2.receipt_no),客戶名稱 = rtrim(s2.cust_name),區域 = rtrim(t2m.city),地址 = rtrim(t1m.address) " & _
                    ",產品別 = '',項次 = '',品號 = '',件數確認 = isnull(t2.otqty,0) + isnull(o2.otqty,0),出貨箱數 = '',出貨總個數 = '',箱入數 = '',單位重 = '',單位材 = '',出貨重 = '',出貨材 = '',換算材 = round( " & _
                    ...
                    "case when rtrim(t2m.city) in ('基隆','苗栗') then 1.1 " & _
                    "when rtrim(t2m.city) in ('台北','新竹') then 1 " & _
                    "when rtrim(t2m.city) = '桃園' then 0.8 else 1 end " & _
                    ",3) " & _
...
Call Confirm_Recordset_Closed(tmp_Rs)
tmp_Rs.Open str_SQL, cn, adOpenForwardOnly, adLockReadOnly
If tmp_Rs.EOF Then MsgBox "查無資料!", 16, Me.Caption: tmp_Rs.Close: GoTo EndProc

Dim rsTmp As New ADODB.Recordset
Call OffLineRecordset(tmp_Rs, rsTmp)
tmp_Rs.Close

 Recordset2Excel "訂單明細", rsTmp
'在此編輯EXCEL
Screen.MousePointer = 11
With MyXlsApp: .Visible = False
...
Do While Not rsTmp.EOF
    ...
    If rsTmp("地址") <> strAddress Then
        txtPointT0 = txtPointT0 + 1
        strAddress = rsTmp("地址")
    End If
    txtTransCubeT0 = txtTransCubeT0 + rsTmp("換算材")
...
If UCase(rsTmp("請款代碼")) <> "STAIRS" And UCase(rsTmp("請款代碼")) <> "CA-R" And UCase(rsTmp("請款代碼")) <> "C-R" And UCase(rsTmp("請款代碼")) <> "C1-R" And UCase(rsTmp("請款代碼")) <> "FULL_CS-R" And UCase(rsTmp("請款代碼")) <> "HT-R" And UCase(rsTmp("請款代碼")) <> "HT1-R" And UCase(rsTmp("請款代碼")) <> "KL-R" And UCase(rsTmp("請款代碼")) <> "KL1-R" And UCase(rsTmp("請款代碼")) <> "ML-R" And UCase(rsTmp("請款代碼")) <> "ML1-R" And UCase(rsTmp("請款代碼")) <> "SA-R" And UCase(rsTmp("請款代碼")) <> "TP-R" And UCase(rsTmp("請款代碼")) <> "TP1-R" And UCase(rsTmp("請款代碼")) <> "TY-R" And UCase(rsTmp("請款代碼")) <> "TY1-R" Then
    
    If UCase(rsTmp("請款代碼")) <> "W-R" And UCase(rsTmp("請款代碼")) <> "FORKLIFT" And UCase(rsTmp("請款代碼")) <> "CANCEL" And UCase(rsTmp("請款代碼")) <> "REPALLETIS" And Left(rsTmp("請款代碼"), 6) <> "002-34" And Left(rsTmp("請款代碼"), 6) <> "002-25" And Left(rsTmp("請款代碼"), 6) <> "000-31" And Left(rsTmp("請款代碼"), 6) <> "001-97" And Left(rsTmp("請款代碼"), 6) <> "002-29" And Left(rsTmp("請款代碼"), 6) <> "001-36" And Left(rsTmp("請款代碼"), 6) <> "000-70" And Left(rsTmp("請款代碼"), 6) <> "000-67" And Left(rsTmp("請款代碼"), 6) <> "001-23" And UCase(Left(rsTmp("請款代碼"), 6)) <> "CANCEL" And rsTmp("請款類別") <> "轉運" And Left(rsTmp("備註"), 3) <> "不分攤" Then
        dbAP = dbAP + rsTmp("應付總價")
    End If
...
dbPremiam = InputBox("不列入A段分攤條件如下：" & vbCr & vbLf & "1.代碼前六碼:002-34,002-29,001-97,000-31,001-36,000-70,000-67,001-23,002-25,Cancel" & vbCr & vbLf & "2.計費代碼:Stairs,forklift,RePalletIs,CA-R,C-R,C1-R,FULL_CS-R,HT-R,HT1-R,KL-R,KL1-R,ML-R,ML1-R,SA-R,TP-R,TP1-R,TY-R,TY1-R,W-R" & vbCr & vbLf & "3.計費類別:轉運" & vbCr & vbLf & "4.備註開頭:不分攤", "請輸入議價金額(輸入0元或按取消可中止計算)", 0, 0)
...
If dgRouteT0.Col = 1 Then
    ...
    If rsRouteT0("選取") = " " Then
    
        rsRouteT0("選取") = "V"
        txtTotalPLT0 = Val(txtTotalPLT0) + rsRouteT0("出貨板數")
        txtTotalCST0 = Val(txtTotalCST0) + rsRouteT0("出貨箱數")
        txtTotalCubeT0 = Val(txtTotalCubeT0) + rsRouteT0("出貨材積")
        txtTotalWgtT0 = Val(txtTotalWgtT0) + rsRouteT0("出貨重量")
    Else
        rsRouteT0("選取") = " "
        txtTotalPLT0 = Val(txtTotalPLT0) - rsRouteT0("出貨板數")
        txtTotalCST0 = Val(txtTotalCST0) - rsRouteT0("出貨箱數")
...
',贈=(select top 1 case when rtrim(cod1.InvoicePCode) = 'N' then '贈' else ' ' end from custorderdetail cod1 where rtrim(cod1.InvoicePCode) = 'N' and rtrim(cod1.ordertype) + rtrim(cod1.externorderkey) = t02t.extern and adddate > getdate()-90 )

str_SQL = "select 路線編號 = t02t.Route_No,到貨日期 = rtrim(t02t.Arrive_Date) ,訂單編號 = Rtrim(t02t.Extern),贈=case when t02t.STORERKEY = 'LMBO01' then (select distinct '贈' from custorderdetail cod1 where rtrim(cod1.InvoicePCode) = 'N' and rtrim(cod1.ordertype) + rtrim(cod1.externorderkey) = t02t.extern ) else '' end " & _
    ",件數 = isnull((select isnull(otqty,0) from trp02t (nolock) where receipt_no = t02t.Receipt_No),0) + isnull((select isnull(otqty,0) from ort02t (nolock) where receipt_no = t02t.Receipt_No),0) " & _
    ...
    ",驗收單號=Isnull(rtrim(t02t.CustomerOrderkey1),''),TMS單號 = rtrim(t02t.Receipt_No),應收代收貨款 = isnull(o.cash,''),實收代收貨款 =  case when isnull(cast(o.receivecash as varchar),'0') = '0' then ' ' else cast(o.receivecash as varchar) end ,計費代碼 = rtrim(isnull(t9m.car_type,'')),車牌號碼 = Rtrim(t01t.c_Vehicle_ID_No) " & _
    ",駕駛人 = Rtrim(t01t.driver),一次車號 = Rtrim(t02t.Vehicle_ID_No),貨主 =Rtrim(t02t.StorerKey) + '_' + rtrim(t16.short_name),訂單類別 = rtrim(t02t.priority) ,配送倉別 = rtrim(o.facility)" & _
    ",客戶名稱 = Rtrim(Isnull(t1m.Short_Name,'')),訂單備註 = Rtrim(Isnull(t02t.Description,'')),送貨地址 = Rtrim(Isnull(t1m.Address,'')) " & _
    ",簽單回傳日期=isnull(t02t.SDNSendDate,getdate()) " & _
...
If dgOrderT0.Col <> 0 And rsOrderT0.Fields(dgOrderT0.Col).Name <> "驗收單號" And rsOrderT0.Fields(dgOrderT0.Col).Name <> "狀態" And rsOrderT0.Fields(dgOrderT0.Col).Name <> "實收代收貨款" Then
    dgOrderT0.Col = 10
End If

If dgOrderT0.Col <> 0 And Len(RTrim(rsOrderT0("狀態"))) > 1 Then
    dgOrderT0.Col = 10
    Exit Sub
End If
...
'Screen.MousePointer = 11
'rsMain.Fields(.Col).Name = "預估到貨"

''僅限特定貨主
If rsOrderT0.Fields(dgOrderT0.Col).Name = "狀態" Then 'And (mySplit(rsOrderT0("貨主"), "_", 0) = "LTKK01" Or mySplit(rsOrderT0("貨主"), "_", 0) = "LVTL01" Or mySplit(rsOrderT0("貨主"), "_", 0) = "LNSL01" Or mySplit(rsOrderT0("貨主"), "_", 0) = "LABT01") Then
...
'驗收單號
If rsOrderT0.Fields(dgOrderT0.Col).Name = "驗收單號" Then Call CustomerOrderkey("驗收單號")
...
With dgOrderT0
    ...
    If Str_Field = "驗收單號" Then
        txtCustomerOrderkey.Height = .RowHeight + 10
        If rsOrderT0.Fields(.Col).Name = "驗收單號" Then
...
str_SQL = "select 二次路編 = t02t.c_Route_No " & _
            ...
            ",訂單類別 = rtrim(t02t.priority),訂單編號 = rtrim(t02t.Receipt_No),C_Receipt_no = rtrim(t02t.C_Receipt_No),訂單日期 = rtrim(t02t.Receipt_Date) " & _
            ...
            ",到貨 = isnull(ontimedelivery,0),簽單已回 = t02t.sdnback,分公司代碼 = rtrim(isnull(co.branchid,'')),客戶單別=rtrim(isnull(co.ordertype,'')),樓層=isnull(co.Stairs,''),計費類別=isnull(t9m.car_type,'') " & _
...
txt_Externordertype.Text = tmp_Rs.Fields("客戶單別").Value
txt_BranchId.Text = tmp_Rs.Fields("分公司代碼").Value
txt_Stairs = tmp_Rs.Fields("樓層").Value
txt_Cartype.Text = tmp_Rs.Fields("計費類別").Value

If Trim(tmp_Rs("郵遞區號")) = "" Then MsgBox "注意!郵遞區號空白!", 16, Me.Caption

If txt_Priority = "R" Or txt_Priority = "RC" Or txt_Priority = "A2B" Then
    txt_OneOrder_ConsigneeKey.Text = tmp_Rs.Fields("客戶編號")
    txt_ZIP.Text = tmp_Rs("郵遞區號")
    txt_OneOrder_FullName.Text = tmp_Rs.Fields("客戶名稱")
    txt_OneOrder_Address.Text = tmp_Rs.Fields("送貨地址")
...
Else
    ...
    txt_OneOrder_ConsigneeKey1 = tmp_Rs.Fields("客戶編號")
    txt_Zip1 = tmp_Rs.Fields("郵遞區號")
    txt_OneOrder_FullName1 = tmp_Rs.Fields("客戶名稱")
    txt_OneOrder_Address1 = tmp_Rs.Fields("送貨地址")
End If
...
str_SQL = "select " & _
          ...
          "應收=o.cash,實收=case when isnull(rtrim(cast(o.receiveCash as char)),'') = '0' then ' ' else  isnull(rtrim(cast(o.receiveCash as char)),'') end ," & _
          "組單類型 = rtrim(isnull(o.B_city,'')) ," & _
          "priority = rtrim(o.priority),facility=case when o.facility = '' then '佰事達北倉' else o.facility end,客戶編號=rtrim(t1.consigneekey) , 客戶名稱=rtrim(t1.full_name) , 郵遞區號=rtrim(isnull(t1.zip,'')) , 送貨地址=rtrim(t1.address) from orders o (nolock) join trp01m t1 (nolock) on t1.storerkey = o.storerkey and t1.consigneekey = case when o.priority = 'A2B' then o.b_company else o.consigneekey end where o.orderkey = '" & txt_C_Receipt_No & "' "
tmp_Rs.Open str_SQL, cn, adOpenForwardOnly, adLockReadOnly

'補資料
    txt_UrgentMark = tmp_Rs.Fields("UrgentMark")
    txt_ReserveMark = tmp_Rs.Fields("ReserveMark")
    txt_Cash.Text = tmp_Rs.Fields("應收")
    txt_ReceiveCash.Text = tmp_Rs.Fields("實收")
    cbx_B_city.Text = tmp_Rs.Fields("組單類型")
...
tmpI = 1
Do While Not tmp_Rs.EOF
   If tmp_Rs.Fields("送貨量").Value = 0 Then bl0Qty = True
   With gd_OneOrder_OrderDetail
...
'更新出貨量
'If bl0Qty = True Then Call Ship2TMS(strExtern) 'Mark @20130819 4 顯示的出貨量(現行是系統)定義為實際出貨量(考量倉庫揀貨正確性，有可能倉庫出多，計費需少計)
