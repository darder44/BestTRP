...
If Len(txtExternOrderkeyS.Text) > 0 And Len(txtExternOrderkeyE.Text) > 0 Then
   chc_txtExternorderkey = "and 訂單號碼 between '" & txtExternOrderkeyS.Text & "' and '" & txtExternOrderkeyE.Text & "' "
ElseIf Len(txtExternOrderkeyS.Text) > 0 And Len(txtExternOrderkeyE.Text) = 0 Then
   chc_txtExternorderkey = "and 訂單號碼 = '" & txtExternOrderkeyS.Text & "' "
ElseIf Len(txtExternOrderkeyS.Text) = 0 And Len(txtExternOrderkeyE.Text) > 0 Then
   chc_txtExternorderkey = "and 訂單號碼 = '" & txtExternOrderkeyE.Text & "' "
End If

'貨主編號
If Len(RTrim(Combo1.Text)) > 0 Then chc_Storerkey = " and 貨主編號 ='" & Combo1.Text & "' "
...
For i = 0 To List1.ListCount - 1
    ...
    End If
Next
If Len(strTmp) > 0 Then str_SQL = str_SQL & "and 入庫完成 in (" & Left(strTmp, Len(strTmp) - 1) & ") "

'組合字串
str_SQL = str_SQL & chc_Orderdate & chc_DeliveryDate & chc_Storerkey & chc_txtExternorderkey & " order by 貨主編號 , 類別 , 到貨日期 , 訂單號碼 , 品號 "
...
If (rsMain("類別") = "退換貨" Or rsMain("類別") = "拒短收") = False Then MsgBox "只有TMS的退換貨與拒短收類別，才能手動更改收貨狀態!", 64, "注意": Exit Sub

'更新狀態
cn.Execute "update sdn02t set ExpectReceiptOK = '" & Trim(cboExpectReceiptOK) & "' where extern = '" & rsMain("訂單號碼") & "' and receipt_no = '" & rsMain("TMS單號") & "' ", RowsAffect, adExecuteNoRecords

If RowsAffect > 0 Then
    rsMain("入庫完成") = Trim(cboExpectReceiptOK)
    MsgBox "TMS單號 " & rsMain("TMS單號") & "，入庫狀態已更新為" & Trim(cboExpectReceiptOK) & "。", 64, "狀態修改"
Else
    MsgBox "更新失敗，請確認此訂單是否已完成出車確認?", 16, "狀態修改"
End If
End Sub
...
If RTrim(Combo1.Text) = "" Then MsgBox "請選取貨主", 64, cmdBatchConfirm.Caption: Exit Sub

If MsgBox("1.WMS已收貨完成之驗收單" & vbCrLf & "2.TMS已完成出車確認" & vbCrLf & "3.交運單號(WMS)=訂單號碼(TMS)", vbOKCancel, "更新條件") = vbOK Then

cn.Execute "exec gs_ExpectReceiptOK '" & RTrim(Combo1.Text) & "' ", RowsAffect, adExecuteNoRecords
MsgBox "更新 " & RowsAffect & "筆訂單資料", , cmdBatchConfirm.Caption
End If
...
cboExpectReceiptOK = Trim(rsMain("入庫完成"))
