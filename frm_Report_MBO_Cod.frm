...
Begin VB.Form frm_Report_MBO_Cod 
   Caption         =   "毛寶下貨收現查詢"
   ClientHeight    =   6300
   ClientLeft      =   60
   ClientTop       =   345
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmdSaveToText 
         ...
         Height          =   870
         Left            =   5880
         Picture         =   "frm_Report_MBO_Cod.frx":0000
         Style           =   1  '圖片外觀
         TabIndex        =   15
         Top             =   1200
         Visible         =   0   'False
      ...
      Begin VB.CommandButton cmd2Excel 
         ...
         Height          =   870
         Left            =   5880
         Picture         =   "frm_Report_MBO_Cod.frx":030A
         Style           =   1  '圖片外觀
         TabIndex        =   3
         Top             =   240
         Width           =   1065
      ...
      Begin VB.CommandButton cmdExit 
         ...
         Height          =   870
         Left            =   7080
         Picture         =   "frm_Report_MBO_Cod.frx":1604
         Style           =   1  '圖片外觀
         TabIndex        =   5
         Top             =   1200
         Width           =   1065
      ...
      Begin VB.CommandButton cmdReset 
         ...
         Height          =   870
         Left            =   7080
         Picture         =   "frm_Report_MBO_Cod.frx":2B216
         Style           =   1  '圖片外觀
         TabIndex        =   4
         Top             =   240
         Width           =   1065
      ...
      Begin VB.CommandButton cmdQuery 
         ...
         Height          =   870
         Left            =   4680
         Picture         =   "frm_Report_MBO_Cod.frx":2B528
         Style           =   1  '圖片外觀
         TabIndex        =   2
         Top             =   240
         Width           =   1065
   ...
   End
End
Attribute VB_Name = "frm_Report_MBO_Cod"
...
str_SQL = "select " & _
            "訂單類型 = RTrim(co.ordertype) " & _
            ",訂單號碼 = RTrim(co.ordertype) + rtrim(co.externorderkey) " & _
            ",需求日 = convert(char(10),co.deliverydate,111) " & _
            ",客戶編號 = rtrim(co.consigneekey) " & _
            ",客戶名稱 = rtrim(co.short_name) " & _
            ",區域代號 = rtrim(co.City) " & _
            ",區域名稱 = rtrim(t2m.description) " & _
            ",地址 = rtrim(co.address) " & _
            ",收現金額 = isnull( cast(rtrim(co.Amount) as float) ,0) " & _
            ",訂單備註 = rtrim(co.notes) " & _
            ",發票號碼= isnull( rtrim(co.invoice),'') " & _
            "from custorders co join orders o on o.orderkey = co.orderkey " & _
            "left join trp02m t2m on t2m.zip = co.City " & _
            "where co.storerkey = 'LMBO01' and o.type <> '刪單' and rtrim(co.Payment) = '.' and co.ordertype in ('SO','SB') "
'訂單日期
chc_Orderdate = ""
