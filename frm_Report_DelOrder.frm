...
Else
    ...
    '    '備份檔案
        If Dir("C:\LTKK01\刪單", vbDirectory) = "" Then MkDirs "C:\LTKK01\刪單"
        .ActiveWorkbook.SaveAs "C:\LTKK01\刪單\刪單_" & Format(Now, "yyyymmddhhMMss") & ".xls"
...
str_SQL = "select 倉別 = 'BL01' " & _
            ...
            ",訂單號碼交貨單號 = rtrim(od.externorderkey) + rtrim(substring(isnull(od.externlineno,''),CHARINDEX('_',isnull(od.externlineno,''))+1,15)) " & _
            ",地址別 = rtrim(substring(o.consigneekey,5,20)) " & _
            ",料號 = rtrim(isnull(ss.storersku , od.sku)) " & _
            ",倉別_儲位別 = 'BL01_'+ rtrim(od.lottable06) " & _
            ...
            "From orders o join orderdetail od on o.orderkey = od.orderkey and o.type = '刪單' " & _
