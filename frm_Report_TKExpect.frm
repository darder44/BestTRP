...
Begin VB.Form frm_Report_TKExpect 
   Caption         =   "訂單退回明細"
   ClientHeight    =   6300
   ClientLeft      =   60
   ClientTop       =   345
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmdSaveToText 
         ...
         Height          =   870
         Left            =   5880
         Picture         =   "frm_Report_TKExpect.frx":0000
         Style           =   1  '圖片外觀
         TabIndex        =   15
         Top             =   1200
         Visible         =   0   'False
      ...
      Begin VB.CommandButton cmd2Excel 
         ...
         Height          =   870
         Left            =   5880
         Picture         =   "frm_Report_TKExpect.frx":030A
         Style           =   1  '圖片外觀
         TabIndex        =   3
         Top             =   240
         Width           =   1065
      ...
      Begin VB.CommandButton cmdExit 
         ...
         Height          =   870
         Left            =   7080
         Picture         =   "frm_Report_TKExpect.frx":1604
         Style           =   1  '圖片外觀
         TabIndex        =   5
         Top             =   1200
         Width           =   1065
      ...
      Begin VB.CommandButton cmdReset 
         ...
         Height          =   870
         Left            =   7080
         Picture         =   "frm_Report_TKExpect.frx":2B216
         Style           =   1  '圖片外觀
         TabIndex        =   4
         Top             =   240
         Width           =   1065
      ...
      Begin VB.CommandButton cmdQuery 
         ...
         Height          =   870
         Left            =   4680
         Picture         =   "frm_Report_TKExpect.frx":2B528
         Style           =   1  '圖片外觀
         TabIndex        =   2
         Top             =   240
         Width           =   1065
   ...
   End
End
Attribute VB_Name = "frm_Report_TKExpect"
...
'    '備份檔案
    If Dir("C:\LTKK01\CRED", vbDirectory) = "" Then MkDirs "C:\LTKK01\CRED"
    .ActiveWorkbook.SaveAs "C:\LTKK01\CRED\CRED_" & Format(Now, "yyyymmddhhMMss") & ".xls"
...
str_SQL = "select 倉別 =case when right(isnull((select top 1 od.lottable06 from orderdetail od join orders o on o.orderkey = od.orderkey where od.externorderkey = s2.extern and s3.seq_no = od.orderlinenumber and s2.priority = o.priority and isnull(o.type,'') <> '刪單'),''),2) = '-S' then 'BL02' else 'BL01' end " & _
        ",貨主代碼 = s2.storerkey " & _
        ",訂單號碼交貨單號 = rtrim(s2.extern) + isnull((select top 1 rtrim(od.externlineno) from orderdetail od join orders o on o.orderkey = od.orderkey where od.externorderkey = s2.extern and s3.seq_no = od.orderlinenumber and s2.priority = o.priority and isnull(o.type,'') <> '刪單'),'') " & _
        ",地址別 = substring(s2.consigneekey,5,20) " & _
        ",料號 = isnull(ss.storersku,s3.product_no) " & _
        ",倉別_儲位別 = isnull((select top 1 od.lottable06 from orderdetail od join orders o on o.orderkey = od.orderkey where od.externorderkey = s2.extern and s3.seq_no = od.orderlinenumber and s2.priority = o.priority and isnull(o.type,'') <> '刪單'),'') " & _
        ",最小單位數量 = isnull(s3.order_qty,0) - isnull(s3.sign_qty,0) " & _
        ",退貨日 = convert(varchar,s2.confirm_date,111) " & _
        "From sdn02t s2 join sdn03t s3 on s2.receipt_no = s3.receipt_no " & _
        "left join storersku ss on ss.sku = s3.product_no and ss.storerkey = s3.storerkey " & _
        "Where IsNull(s3.order_qty, 0) - IsNull(s3.sign_qty, 0) > 0 " & _
        "and len(s2.confirm_date) > 0 and s2.priority <> 'R' " & _
        "and s2.confirm_notes <> '正常訂單' "
    
'簽單日期
chc_Orderdate = ""
If Len(txtSDNDateS.Text) > 0 And Len(txtSDNDateE.Text) > 0 Then
   chc_Orderdate = "and convert(Char(8),s2.confirm_date,112) between '" & txtSDNDateS.Text & "' and '" & txtSDNDateE.Text & "' "
ElseIf Len(txtSDNDateS.Text) > 0 And Len(txtSDNDateE.Text) = 0 Then
   chc_Orderdate = "and convert(Char(8),s2.confirm_date,112) = '" & txtSDNDateS.Text & "' "
ElseIf Len(txtSDNDateS.Text) = 0 And Len(txtSDNDateE.Text) > 0 Then
   chc_Orderdate = "and convert(Char(8),s2.confirm_date,112) = '" & txtSDNDateE.Text & "' "
End If
...
'貨主
If Len(RTrim(Combo1.Text)) > 0 Then chc_Storerkey = " and s2.storerkey ='" & Combo1.Text & "' "

'組合字串
str_SQL = str_SQL & chc_Orderdate & chc_DeliveryDate & chc_Storerkey & "order by s2.consigneekey "
...
Do While Not rsMain.EOF
    ...
    '更新為已回傳
    cn.Execute "update " & strWMSDB & "..orders set yfystatus = '2' ,TrafficCop = null where orderkey = '" & strM & "' and storerkey = 'LTKK01'", RowsAffect, adExecuteNoRecords
...
End Sub
Private Sub txtSDNDateS_Click()

Set objMvdateTarget = txtSDNDateS
mvDate.Move objMvdateTarget.Left + objMvdateTarget.Width, objMvdateTarget.Top + objMvdateTarget.Height
mvDate.Visible = True: mvDate.Value = Now
    
End Sub
Private Sub txtSDNDateE_Click()

Set objMvdateTarget = txtSDNDateE
mvDate.Move objMvdateTarget.Left + objMvdateTarget.Width, objMvdateTarget.Top + objMvdateTarget.Height
mvDate.Visible = True: mvDate.Value = Now
    
End Sub
Private Sub txtSDNDateS_KeyPress(KeyAscii As Integer)

If KeyAscii = 27 Then mvDate.Visible = False

End Sub

Private Sub txtSDNDateE_KeyPress(KeyAscii As Integer)
