...
str_SQL = "select 訂單類別 = case o2t.priority when 'RC' then '提貨入庫單' when 'A2B' then '提貨配送單' else case when o2t.storerkey = 'LTKK01' and substring(o2t.extern,3,2) = '12' then '退貨單(換貨)' else '退貨單' end end " & _
        ...
        ", 備註 = case when len(cast(o.notes as varchar(1000))) > 0 or len(cast(od.notes as varchar(1000))) > 0 then cast(isnull(o.notes,'') as varchar(1000)) + '_' + cast(isnull(od.notes,'') as varchar(1000)) else ' ' end , 總個數= o3t.order_qty " & _
...
On Error GoTo err_Handle
str_SQL = "Select 出車日期,車次,區域,路線編號,[二次車號(A)],[二次駕駛(A)],[一次車號(B)],[一次駕駛(B)] " & _
          ",運送板數,運送箱數,運送個數,運送重量,運送材積,[提貨客戶(A)],[到貨客戶(B)],備註 " & _
...
Else
   ...
   Exit Sub
End If
str_SQL = str_SQL & " order by 出車日期,[一次車號(B)],車次,區域 "
...
'    MyXlsApp.Cells(i, 19).Value = "隔板"
    ...
    MyXlsApp.Cells(i, 1).Value = "編號"
    MyXlsApp.Cells(i, 2).Value = "出車日期"
    MyXlsApp.Cells(i, 3).Value = "車次"
    MyXlsApp.Cells(i, 4).Value = "區域"
    MyXlsApp.Cells(i, 5).Value = "路線編號"
    MyXlsApp.Cells(i, 6).Value = "二次車號(A)"
    MyXlsApp.Cells(i, 7).Value = "二次駕駛(A)"
    MyXlsApp.Cells(i, 8).Value = "一次車號(B)"
    MyXlsApp.Cells(i, 9).Value = "一次駕駛(B)"
    MyXlsApp.Cells(i, 10).Value = "運送板數"
    MyXlsApp.Cells(i, 11).Value = "運送箱數"
    MyXlsApp.Cells(i, 12).Value = "運送個數"
    MyXlsApp.Cells(i, 13).Value = "運送重量"
    MyXlsApp.Cells(i, 14).Value = "運送材積"
    MyXlsApp.Cells(i, 15).Value = "提貨客戶(A)"
    MyXlsApp.Cells(i, 16).Value = "到貨客戶(B)"
    MyXlsApp.Cells(i, 17).Value = "備註"
    MyXlsApp.Cells(i, 18).Value = "追蹤時間"
    MyXlsApp.Cells(i, 19).Value = "借出"
    MyXlsApp.Cells(i, 20).Value = "回收"
    ...
    Do While Not rsMain1.EOF
        If i > 2 Then
            If MyXlsApp.Cells(i - 1, 4).Value <> rsMain1.Fields(8) Then
                '車號不同,隔一行在寫入excel
                MyXlsApp.Cells(i, 11).Value = "=SUM(K" & CStr(j) & ":K" & CStr(i - 1) & ")"  '箱數
                MyXlsApp.Cells(i, 12).Value = "=SUM(L" & CStr(j) & ":L" & CStr(i - 1) & ")"  '個數
                MyXlsApp.Cells(i, 13).Value = "=SUM(M" & CStr(j) & ":M" & CStr(i - 1) & ")" '運送重量
                MyXlsApp.Cells(i, 14).Value = "=SUM(N" & CStr(j) & ":N" & CStr(i - 1) & ")" '材積
                i = i + 2
                j = i
            End If
        End If
        ...
        MyXlsApp.Cells(i, 2).Value = Trim(rsMain1.Fields(1)) '出車
        MyXlsApp.Cells(i, 3).NumberFormatLocal = "@"
        MyXlsApp.Cells(i, 3).Value = rsMain1.Fields(2) '車次
        MyXlsApp.Cells(i, 4).NumberFormatLocal = "@"
        MyXlsApp.Cells(i, 4).Value = rsMain1.Fields(3) '區域
        MyXlsApp.Cells(i, 5).NumberFormatLocal = "@"
        MyXlsApp.Cells(i, 5).Value = rsMain1.Fields(4) '路編
        MyXlsApp.Cells(i, 6).NumberFormatLocal = "@"
        MyXlsApp.Cells(i, 6).Value = rsMain1.Fields(5) '二次車號
        MyXlsApp.Cells(i, 7).Value = rsMain1.Fields(6) '二次駕駛
        MyXlsApp.Cells(i, 8).NumberFormatLocal = "@"
        MyXlsApp.Cells(i, 8).Value = rsMain1.Fields(7) '一次車號
        MyXlsApp.Cells(i, 9).Value = rsMain1.Fields(8) '一次駕駛
        MyXlsApp.Cells(i, 10).Value = rsMain1.Fields(9) '運送板數
        MyXlsApp.Cells(i, 11).Value = rsMain1.Fields(10) '運送箱數
        MyXlsApp.Cells(i, 12).Value = rsMain1.Fields(11) '運送個數
        MyXlsApp.Cells(i, 13).Value = rsMain1.Fields(12) '運送重量
        MyXlsApp.Cells(i, 14).Value = rsMain1.Fields(13) '運送材積
        MyXlsApp.Cells(i, 15).NumberFormatLocal = "@"
        MyXlsApp.Cells(i, 15).Value = rsMain1.Fields(14) '客戶簡稱A
        MyXlsApp.Cells(i, 16).NumberFormatLocal = "@"
        MyXlsApp.Cells(i, 16).Value = rsMain1.Fields(15) '客戶簡稱B
        MyXlsApp.Cells(i, 17).NumberFormatLocal = "@"
        MyXlsApp.Cells(i, 17).Value = rsMain1.Fields(16) '備註
        rsMain1.MoveNext
        i = i + 1
    Loop
                MyXlsApp.Cells(i, 11).Value = "=SUM(K" & CStr(j) & ":K" & CStr(i - 1) & ")"  '箱數
...
str_SQL = "select 貨主,出車日,暫存碼頭,客戶,品名,品號,條碼,箱入數,箱數,'確認1' = 確認,'確認2' = 確認,'確認3' = 確認 from ##DivideSku where 箱數 > 0 order by  出車日,品名,客戶,暫存碼頭"
'分貨表箱
Call Confirm_Recordset_Closed(tmp_Rs)
tmp_Rs.Open str_SQL, cn, adOpenForwardOnly, adLockReadOnly
'If tmp_Rs.EOF Then
...
'分貨表個
str_SQL = "select 貨主,出車日,暫存碼頭,客戶,品名,品號,條碼,箱入數,個數,'確認1'=確認,'確認2'=確認,'確認3' = 確認 from ##DivideSku where 個數 > 0  order by  出車日,品名,客戶,暫存碼頭"
Call Confirm_Recordset_Closed(tmp_Rs)
tmp_Rs.Open str_SQL, cn, adOpenForwardOnly, adLockReadOnly
'If tmp_Rs.EOF Then
'   Screen.MousePointer = vbDefault
...
'If Int_loc >= 106 Then
'            msg_text = "超出暫存區儲位(N開頭的)，請轉出時，手動修改"
'            MsgBox msg_text, vbOKOnly + vbInformation, msg_title
'End If
...
Do While Not in_rs.EOF
    ...
    End If
         ...
         If Trim(in_rs.Fields("暫存碼頭").Value) <> Str_Sku Then
            ...
            Dob_CS = Trim(in_rs.Fields("箱數").Value): Dob_EA = Trim(in_rs.Fields("個數").Value): Dob_Total = Trim(in_rs.Fields("總數量").Value)
        ...
        Else
            ...
            Dob_Total = Dob_Total + Trim(in_rs.Fields("總數量").Value)
        End If
